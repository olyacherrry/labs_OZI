...
namespace Lab3
{
    ...
    class S_DES
    {
        ...
        public static Tuple<List<BitArray>, String> MakeS_DES(String bitTextInStr, String keyText, Boolean type)
        {
            ...
            log.Append(String.Format("{0, -5}: {1}", "KEY_", ToString(keyBit))).Append(Environment.NewLine);
            ...
            MoveLeft(key, 1);
            log.Append(String.Format("{0, -5}   : {1}", "<-1", ToString(key))).Append(Environment.NewLine);
            BitArray k1 = RefreshArray(key, Permutations.P8);
            log.Append(String.Format("{0, -5}: {1}", "K1(P8)", ToString(k1))).Append(Environment.NewLine);
            MoveLeft(key, 2);
            log.Append(String.Format("{0, -5}   : {1}", "<-2", ToString(key))).Append(Environment.NewLine);
            BitArray k2 = RefreshArray(key, Permutations.P8);
            log.Append(String.Format("{0, -5}: {1}", "K2(P8)", ToString(k2))).Append(Environment.NewLine).Append(Environment.NewLine);
            if (!type)
            {
            ...
            Int32 blockLength = 8;
            for(int i = 0; i < bitTextInStr.Length; i += blockLength)
            {
                String part = bitTextInStr.Substring(i, blockLength);
                BitArray textBit = new BitArray(blockLength);
                for (int j = 0; j < blockLength; j++)
                {
                    textBit.Set(j, part[j].Equals('1') ? true : false);
                }
                log.Append(String.Format("{0, -5} : {2}", "Block", part.ToString(), ToString(textBit))).Append(Environment.NewLine);
                result.Add(MakeTransformation(textBit, k1, k2));
            }
            return Tuple.Create(result, log.ToString());
        }
        ...
        private static BitArray MakeTransformation(BitArray openTextBitArray, BitArray key1,  BitArray key2)
        {
            ...
            log.Append(String.Format("{0, -5}: {1}", "RND1", ToString(round1))).Append(Environment.NewLine); 
            SwapParts(round1);
            log.Append(String.Format("{0, -5}  : {1}", "SW", ToString(round1))).Append(Environment.NewLine);
            BitArray round2 = MakeRound(round1, key2);
            log.Append(String.Format("{0, -5}: {1}", "RND2", ToString(round2))).Append(Environment.NewLine);
        ...
        private static BitArray MakeRound(BitArray blockBitArray, BitArray key)
        {
            ...
            log.Append(String.Format("{0, -5}  : {1}", "E_P", ToString(e_p))).Append(Environment.NewLine);
            BitArray _ = e_p.Xor(key);
            log.Append(String.Format("{0, -5}  : {1}", "XOR", ToString(_))).Append(Environment.NewLine);
            tuple = Divide(_);
